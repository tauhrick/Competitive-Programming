...
struct Query {
  ...
  Query(int _k, int _l, int _r, int _u, int _ind, int _typ) {
    k = _k;
    l = _l;
    r = _r;
    u = _u;
...
int base[N];
long long res;
Query queries_less[4 * N], queries_more[4 * N];
int ctr_less, ctr_more;
...
void dfs_st(int u, int p = 0) {
  for (int i = 0; i < (int) G[u].size(); ++i) {
    int v = G[u][i];
    if (v == p) {
      queries_less[++ctr_less] = Query(u, 1, n, u, i, 1);
      queries_more[++ctr_more] = Query(u, 1, n, u, i, 1);
    } else {
      queries_less[++ctr_less] = Query(u, range[v].first, range[v].second, u, i, 1);
      queries_more[++ctr_more] = Query(u, range[v].first, range[v].second, u, i, 1);
    }
  }
  for (auto v : G[u]) {
    if (v != p) {
...
void test_case() {
  ...
  dfs_pre(1);
  for (int i = 1; i <= n; ++i) {
    g[i].resize((int) G[i].size());
    queries_less[++ctr_less] = Query(base[i], INT_MAX, INT_MAX, INT_MAX, i, -1);
    queries_more[++ctr_more] = Query(base[i], INT_MIN, INT_MIN, INT_MIN, i, -1);
  }
  dfs_st(1);
  sort(queries_less + 1, queries_less + 1 + ctr_less);
  Fenwick cnt_less(n + 1);
  for (int i = 1; i <= ctr_less; ++i) {
    Query foo = queries_less[i];
    if (foo.typ == -1) {
      cnt_less.modify(foo.ind, 1);
    } else {
      g[foo.u][foo.ind][0] = cnt_less.get(foo.r) - cnt_less.get(foo.l - 1);
    }
  }
  sort(queries_more + 1, queries_more + 1 + ctr_more);
  Fenwick cnt_more(n + 1);
  for (int i = ctr_more; i >= 1; --i) {
    Query foo = queries_more[i];
    if (foo.typ == -1) {
      cnt_more.modify(foo.ind, 1);
    } else {
      g[foo.u][foo.ind][1] = cnt_more.get(foo.r) - cnt_more.get(foo.l - 1);
    }
  }
  dfs_en(1);
  writeln(res);
...
//~ Attribution: https://codeforces.com/blog/entry/45835
